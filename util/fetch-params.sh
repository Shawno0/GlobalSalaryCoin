...
if [ -n "${1:-}" ]; then
    ...
    if [[ "$OSTYPE" == "darwin"* ]]; then
        PARAMS_DIR="$HOME/Library/Application Support/GlobalSalaryCoinParams"
    else
...
Failed to fetch the GlobalSalaryCoin zkSNARK parameters!
...
function main() {
    ...
    cat <<EOF
GlobalSalaryCoin - fetch-params.sh

This script will fetch the GlobalSalaryCoin zkSNARK parameters and verify their
...
EOF
    ...
    then
        ...
        cat >> "$README_PATH" <<EOF
This directory stores common GlobalSalaryCoin zkSNARK parameters. Note that it is
