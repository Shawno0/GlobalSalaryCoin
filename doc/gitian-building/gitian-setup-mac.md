...
The build script we'll be using is contained in the GlobalSalaryCoin github repository ([contrib/gitian-build.py](https://github.com/globalsalarycoin-project/globalsalarycoin/blob/master/contrib/gitian-build.py)). Since this is a completely fresh environment, we haven't yet cloned the GlobalSalaryCoin repository and will need to fetch this script with the following commands:

```bash
curl -L -O https://raw.githubusercontent.com/GlobalSalaryCoin-Project/GlobalSalaryCoin/master/contrib/gitian-build.py
...
- Clone the gitian-builder, gitian.sigs, globalsalarycoin-detached-sigs, and globalsalarycoin GitHub repos.
