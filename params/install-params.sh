...
function install_params {
    ...
    if ! [ -f "$filename" ]
    then
        filename="share/globalsalarycoin/$filename"
    fi
...
function main() {
    ...
    cat <<EOF
GlobalSalaryCoin - install-params.sh

This script will install the GlobalSalaryCoin zkSNARK parameters and verify their
