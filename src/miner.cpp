...
bool ProcessBlockFound(const std::shared_ptr<const CBlock>& pblock, CWallet& wallet, Optional<CReserveKey>& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != g_best_block)
            return error("GlobalSalaryCoinMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, nullptr, pblock, nullptr)) {
        return error("GlobalSalaryCoinMiner : ProcessNewBlock, block not accepted");
    }
...
{
    LogPrintf("GlobalSalaryCoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    util::ThreadRename("globalsalarycoin-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        LogPrintf("Running GlobalSalaryCoinMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void static ThreadBitcoinMiner(void* parg)
{
    ...
    try {
        ...
        LogPrintf("GlobalSalaryCoinMiner exception");
    } catch (...) {
        LogPrintf("GlobalSalaryCoinMiner exception");
    }

    LogPrintf("GlobalSalaryCoinMiner exiting\n");
}
