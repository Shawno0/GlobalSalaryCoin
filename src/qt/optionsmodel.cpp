...
void OptionsModel::setDisplayDefaultOptions(QSettings& settings, bool reset)
{
    ...
    if (settings.contains("nAnonymizePivxAmount") || reset)
        gArgs.SoftSetArg("-anonymizeglobalsalarycoinamount", settings.value("nAnonymizePivxAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in globalsalarycoin.cpp during shotdown to also remove the window positions
